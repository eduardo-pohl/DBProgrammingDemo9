...
namespace DBProgrammingDemo9
{
    public partial class frmProductMaintenance : Form
    {
        ...
        {
            if (MessageBox.Show("Are you sure you wanted to delete?", "Are you sure?", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                DeleteProduct();
            }
        }
        ...
        {
            string sqlInsertQuery = $@"Insert into products (ProductName, SupplierID, CategoryID,QuantityPerUnit,UnitPrice, UnitsInStock, UnitsOnOrder,ReorderLevel,Discontinued)
Values('{DataAccess.replaceSQL(txtProductName.Text.Trim())}', {cmbSuppliers.SelectedValue}, {cmbCategories.SelectedValue}, '{DataAccess.replaceSQL(txtQtyPerUnit.Text.Trim())}', {txtUnitPrice.Text.Trim()}, {txtStock.Text.Trim()}, {txtOnOrder.Text.Trim()}, {txtReorder.Text.Trim()}, {(chkDiscontinued.Checked ? 1 : 0)})";
